...
#define BLOCKER_SEARCH_SIZE 0.01
...
float PCSS(sampler2D shadowMap, vec4 shadowCoord) {
  ...
  penumbraRatio =
  max(penumbraRatio, 0.0);

  penumbraRatio =
  penumbraRatio * penumbraRatio;

  float filterRadiusUV =
    penumbraRatio * LIGHT_SIZE_UV * NEAR_PLANE / zReceiver * 2.0;
...
void main(void) {
  ...
  float finalVisibility = mix(shadowStrength, 1.0, visibility);

  gl_FragColor = vec4(phongColor * finalVisibility, 1.0);
}