                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int l = 0; l <= SHOrder; l++)
                    {
                        for (int m = -l; m <= l; m++)
                        {
                            ...
                            SHCoeffiecents[coeffIndex] +=
                                Le * (float)shValue * weight;
                        }
                    }
                }
            }
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                Vector3f wiN = wi.normalized();

                double H = normal.dot(wiN);
                ...
                {
                    Ray3f shadowRay(v + normal * 1e-3f, wiN);
            ...
            if (i == 0)
            {
                std::cout << "Vertex0 coeffs:" << std::endl;
                for (int k = 0; k < shCoeff->size(); k++)
                {
                    std::cout << k << " = " << (*shCoeff)[k] << std::endl;
                }
            }
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            Eigen::MatrixXf directSH = m_TransportSHCoeffs;

            for (int bounce = 0; bounce < m_Bounce; bounce++)
            {
                std::cout << "Computing interreflection bounce " << bounce + 1 << std::endl;

                Eigen::MatrixXf previousSH = m_TransportSHCoeffs;
                Eigen::MatrixXf indirectSH =
                    Eigen::MatrixXf::Zero(SHCoeffLength, mesh->getVertexCount());

                std::mt19937 rng(1337 + bounce);
                ...
                for (int i = 0; i < mesh->getVertexCount(); i++)
                {
                    ...
                    for (int s = 0; s < m_SampleCount; s++)
                    {
                        ...
                        Vector3f up = std::abs(normal.z()) < 0.999f
                        ...
                        float H = normal.dot(wi);

                        Ray3f ray(v + normal * 1e-3f, wi);

                        Intersection its;
                        if (scene->rayIntersect(ray, its))
                        {
                            uint32_t idx0 = its.tri_index.x();
                            uint32_t idx1 = its.tri_index.y();
                            uint32_t idx2 = its.tri_index.z();

                            const Vector3f& bary = its.bary;

                            Eigen::VectorXf hitSH =
                                bary.x() * previousSH.col(idx0) +
                                bary.y() * previousSH.col(idx1) +
                                bary.z() * previousSH.col(idx2);

                            indirectSH.col(i) += (H / M_PI) * hitSH;
                        }
                    }

                    indirectSH.col(i) *= (2.0f * M_PI / float(m_SampleCount));
                }

                m_TransportSHCoeffs = directSH + indirectSH;
            }
        }
