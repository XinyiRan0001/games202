				...
				.load(name + '.obj', function (object) {
					object.traverse(function (child) {
						if (child.isMesh) {
							...
							material.then((data) => {
								console.log("material built:", objMaterial);
								let meshRender = new MeshRender(renderer.gl, mesh, data);
								console.log("mesh render created:", objMaterial);
								renderer.addMeshRender(meshRender);
								console.log("mesh added:", renderer.meshes.length);
							}).catch((err) => {
								console.error("material build failed:", objMaterial, err);
							});

							if (renderer.lights[0].entity.hasShadowMap == true && shadowMaterial != null) {
